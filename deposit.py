...
def deposit_action(amount, user_id, window, back_func):
    try:
        amount = float(amount)
        if amount <= 0:
...
def open_deposit(user_id, balance_label, back_func):
    window = tk.Toplevel()
    ...
    # Section header
    section_header = tk.Label(main_frame, text="Deposit Money", font=("Helvetica", 18, "bold"), fg="#34495e", bg="white")
    ...
    tk.Label(entry_frame, text="Amount to deposit:", font=("Arial", 13), bg="white").pack()
    ...
    # Deposit button
    deposit_btn = tk.Button(button_frame, text="Deposit", 
                        command=lambda: deposit_action(amount_entry.get(), user_id, window, back_func),
                        bg="#2C3E50", fg="white",
                        font=("Arial", 12),
                        relief=tk.FLAT,
                        width=20,
                        height=2)
    deposit_btn.pack(pady=5, expand=True)
